...
static void BM_ParallelProd(benchmark::State& state) {
    for (auto _ : state) {
        for (auto _ : state) {
            state.PauseTiming();
            auto A = TMatrix<double>::CreateRandom(state.range(0), state.range(0));
            auto B = TMatrix<double>::CreateRandom(state.range(0), state.range(0));
            state.ResumeTiming();
            TMatrix<double>::ParallelProd(A, B);
        }
    }
}

// BENCHMARK(BM_ParallelProd)->Arg(1 << 6)->Arg(1 << 7)->Arg(1 << 8)->Arg(1 << 9)->Arg(1 << 10)->Arg(1 << 11)->Arg(1 << 12);
BENCHMARK(BM_ParallelProd)->Arg(1 << 14);
