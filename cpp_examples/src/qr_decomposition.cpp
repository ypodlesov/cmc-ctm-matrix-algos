...
bool QRDecomposition(const TMatrix<double>& A, TMatrix<double>& Q, TMatrix<double>& R) {
    ...
    for (size_t k = 0; k < n; ++k) {
        ...
        for (size_t j = k + 1; j < n; ++j) {
            R(k, j) += TMatrix<double>::InnerProd(Q, k, Q, j);
...
bool QRDecompositionBlockOptimized(TMatrix<double>& A, TMatrix<double>& Q, TMatrix<double>& R) {
    ...
    R.Nullify();
    for (size_t k = 0; k < n; ++k) {
        TVector<double> kthAColumn(A, k);
        double norm = TVector<double>::Norm2(kthAColumn);
        R(k, k) = norm;
        TVector<double> v(Q, k);
        for (size_t i = 0; !RoughEq(0.0, norm) && i < m; ++i) {
            v(i) = kthAColumn(i) / norm;
        }
        for (size_t j = k + 1; j < n; ++j) {
            TVector<double> u(A, j);
            double rkj = 0.0;
